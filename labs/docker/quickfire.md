...
C) To manage container networks
...
B) CMD provides defaults that can be overridden; ENTRYPOINT sets the main command
...
B) Reducing final image size by using intermediate build stages
C) Building multiple images at once
D) Creating image backups
...
D) To prevent image pushing

### 8. How do you tag an image for pushing to a registry?

A) docker tag image-name registry/repository:tag
B) docker push --tag registry/repository:tag
C) docker rename image-name registry/repository:tag
D) docker label image-name registry/repository:tag

### 9. What does the RUN instruction do in a Dockerfile?

A) Runs the container
B) Executes commands during image build and commits the results
C) Sets the default command for the container
D) Defines runtime environment
...
C) An encrypted image
D) An optimized compressed image
...
3. **B** - CMD provides default arguments that can be overridden at runtime. ENTRYPOINT sets the main command that always runs. They can be used together.

4. **A** - Both `COPY` and `ADD` copy files into the image. `COPY` is preferred for simple file copying; `ADD` has additional features (URL support, auto-extraction).

5. **B** - Multi-stage builds use multiple `FROM` instructions to create intermediate stages. The final stage can copy artifacts from earlier stages, reducing final image size.

6. **B** - `docker build -t myimage .` builds an image from the Dockerfile in the current directory, tagging it as "myimage".

7. **B** - The `.dockerignore` file excludes files and directories from the build context, reducing build time and image size (similar to `.gitignore`).

8. **A** - `docker tag source-image registry/repository:tag` creates a new tag. Example: `docker tag myapp docker.io/username/myapp:v1.0`.

9. **B** - `RUN` executes commands during the build process and commits the results as a new image layer. Example: `RUN apt-get update && apt-get install -y curl`.
